...
## ✨ 核心特性
  ...
  * **高性能连接池机制**：内部实现线程安全的 TCP 长连接池，并独创了“借出时心跳探测”机制，彻底消除高并发下的端口耗尽（TIME\_WAIT）问题。
...
| :--- | :--- | :--- | :--- | :--- |
| **v1.0** | 短连接 + 同步调用 | 1,000,000 | 100% | **\~13,000+** |
| **v2.0** | **TCP 长连接池 + 心跳保活** | 1,000,000 | 100% | **\~33,000+** |

![test](test.png)

> **v2.0 性能飞跃原因**：彻底抛弃了每次 RPC 调用的 TCP 三次握手与四次挥手开销，通过连接池复用文件描述符，将网络开销降至最低。
